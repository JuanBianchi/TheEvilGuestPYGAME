...
class Traps(pygame.sprite.Sprite):
    ...
    def load_image(self, trap_type):
        if trap_type == "spikes":
            self.__trap_img = [pygame.image.load("./assets/img/traps/spikes/spikes_long.png")]
            self.__trap_img = [pygame.transform.scale(img, (self.__width, self.__height)) for img in self.__trap_img]
        elif trap_type == "saw":
            self.__trap_img = sfm.get_surface_from_spritesheet("./assets/img/traps/saw/saw.png", 4, 1)
            self.__trap_img = [pygame.transform.scale(img, (self.__width, self.__height)) for img in self.__trap_img]
