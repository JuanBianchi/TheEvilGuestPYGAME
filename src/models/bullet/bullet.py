...
class Bullet(pygame.sprite.Sprite):
    ...
    def load_img(self, bullet_type, direction):
        if bullet_type == 'normal':
            if direction == "Right":
                self.__bullet_img = [pygame.image.load("./src/assets/img/bullet/bullet_r.png")]
            elif direction == "Left":
                self.__bullet_img = [pygame.image.load("./src/assets/img/bullet/bullet_l.png")]
        elif bullet_type == 'hatchet':
            if direction == 'Right':
                self.__bullet_img = sfm.get_surface_from_spritesheet("./src/assets/img/projectile/hatchet/hatchet.png", 7, 1)
                self.__bullet_img = [pygame.transform.scale(img, (50, 20)) for img in self.__bullet_img]
            elif direction == 'Left':
                self.__bullet_img = sfm.get_surface_from_spritesheet("./src/assets/img/projectile/hatchet/hatchet.png", 7, 1, flip=True)
                self.__bullet_img = [pygame.transform.scale(img, (50, 20)) for img in self.__bullet_img]
